...
import { STAT_TIERS as TIERS, STAT_LEVELS as LEVELS, SKILL_RANKS, TIER_VALUES } from "@/components/gameConstants";
...
export default function CompanionsSection({ character, setCharacter }) {
  const getMaxCompanions = () => {
    ...
    const tierIndex = TIERS.indexOf(charismaStat.tier);
  ...
  <CardHeader className="flex items-center justify-between p-6">
          <div>
            ...
            <p className="text-sm text-slate-400 mt-1">
              {currentCompanions} / {maxCompanions} companions (Charisma Tiers - 1) 
            </p>
          </div>
          <Button
      ...
      <Card className="bg-slate-800/50 border-slate-700">
        ...
        <CardContent className="space-y-2 text-sm text-slate-400">
          <p>• <span className="text-amber-400">Limit:</span> Max companions = Charisma Tiers - 1</p>
