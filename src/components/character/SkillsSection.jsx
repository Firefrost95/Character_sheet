...
import { SKILL_RANKS, RANK_MODIFIERS } from "@/components/gameConstants";

const RANKS = SKILL_RANKS.map((name, index) => ({
  name,
  modifier: index === 0 ? 1 : index === 1 ? 0 : index === 2 ? -1 : index === 3 ? -3 : index === 4 ? -4 : index === 5 ? -6 : index === 6 ? -9 : -13
}));
...
export default function SkillsSection({ character, setCharacter }) {
  ...
  const calculateSkillDice = (skill) => {
    ...
    // Minimum dice floor (except for Vermin/Cantrip tiers)
    const stat1Tier = stat1?.tier || "";
    const stat2Tier = stat2?.tier || "";
    const isLowTier = (stat1Tier === "Vermin" || stat1Tier === "Cantrip") && 
                      (!stat2 || stat2Tier === "Vermin" || stat2Tier === "Cantrip");
    
    if (baseDice < 10 && !isLowTier) {
      tModifier += 1;
      wasBuffed = true;
      if (baseDice <= 5) tModifier += 1;
      displayDice = 10; // Display minimum 10 dice
    } else if (baseDice < 10) {
      // For Vermin/Cantrip, still apply penalties but don't force minimum
      tModifier += 1;
      wasBuffed = true;
      if (baseDice <= 5) tModifier += 1;
    }
    ...
    if (displayDice > 25) {
      ...
      // Non-soak rolls cannot drop below t3
      if (tModifier < 3) {
