...
import { Upload, Loader2, User, Plus, Trash2 } from "lucide-react";
import { toast } from "sonner";
import { Checkbox } from "@/components/ui/checkbox";

const MASS_TIERS = ["Feather", "Airy", "Light", "Solid", "Heavy", "Dramatic", "Absurd"];
const LEVELS = ["Low", "Mid", "High"];
const SIZES = ["Miniscule", "Tiny", "Small", "Medium", "Large", "Huge", "Gargantuan", "Titanic"];
...
export default function CharacterBasics({ character, setCharacter }) {
  ...
  const handlePhotoUpload = async (e) => {
    ...
    try {
      ...
      setCharacter({ ...character, photo_url: file_url });
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <Card className="bg-slate-800/50 border-slate-700">
          ...
          <CardContent className="space-y-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => setCharacter({
                  ...
                  growth_points: Math.ceil((parseInt(e.target.value) || 0) / 2)
                })}
                className="bg-slate-700/50 border-slate-600 text-white"
              />
            ...
            <div className="space-y-2">
              <Label className="text-slate-300">Growth Points (GP)</Label>
              <div className="bg-gradient-to-r from-emerald-900/50 to-emerald-800/50 border border-emerald-700/50 rounded-md px-3 py-2">
                <span className="text-2xl font-bold text-emerald-400">
                  {character.growth_points}
                </span>
              </div>
              <p className="text-xs text-slate-400">
                Auto-calculated (½ SP, rounded up). For stats/skills/upgrades only (NOT new powers).
              </p>
            </div>
          </CardContent>
        </Card>
