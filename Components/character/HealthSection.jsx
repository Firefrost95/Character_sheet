      ...
      <Card className="bg-slate-800/50 border-slate-700">
        <CardHeader>
          <CardTitle className="text-amber-100 flex items-center gap-2">
            ...
            Durability & Soak/Resolve
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid md:grid-cols-2 gap-4">
            <div className="space-y-2">
              <Label className="text-slate-300">Durability Type</Label>
              <Input
                value={character.health?.durability_type || ""}
                onChange={(e) => setCharacter({
                  ...character,
                  health: { ...character.health, durability_type: e.target.value }
                })}
                className="bg-slate-700/50 border-slate-600 text-white"
                placeholder="e.g., Lesser Everything Toughness"
              />
